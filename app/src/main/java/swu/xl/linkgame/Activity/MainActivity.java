...
//        UserHelper.logout();
    ...
    private void initChuanshanjia() {
        TTAdSdk.init(this, new TTAdConfig.Builder().appId("5552403")
...
//                        .updateAdConfig(ttAdConfig)//参数类型为TTAdConfig；注意使用该方法会覆盖之前初始化sdk的配置的data值；个性化推荐设置详见：https://www.csjplatform.com/supportcenter/26234
    ...
    private class AdLoadListener implements TTAdNative.RewardVideoAdListener {
        ...
        private final TTAdInteractionListener mInteractionListener = (code, map) -> {
...
//                    Timber.i( "授权成功 --> uid：" + uid);
        ...
        public void onError(int code, String message) {
            ...
            CrashReport.postCatchedException(new Throwable(s));
            Timber.e("Callback --> onError1: %s", s);
//            ToastUtils.show("" + s);
        }
...
//            TToast.show(mActivity, "rewardVideoAd cached 广告类型：" + getAdType(ad.getRewardVideoAdType()));
        ...
        public void handleAd(TTRewardVideoAd ad) {
            ...
            mAd = ad;
            //【必须】广告展示时的生命周期监听
            MediationAdEcpmInfo epm = mAd.getMediationManager().getShowEcpm();
            Timber.d("epm.getEcpm()" + epm.getEcpm());
...
//            TToast.show(mContextRef.get(), "rewardVideoAd show");
            ...
            if (mAdLoadListener == null || mAdLoadListener.mAd == null) {
            }
...
//            TToast.show(mContextRef.get(), "ad onRewardArrived valid:" + isRewardValid + " type:" + rewardType + " errorCode:" + rewardBundleModel.getServerErrorCode());
            if (!isRewardValid) {
    ...
    private void reqADlog() {
        String token = MmkvUtil.getString(MmkvUtil.Token, "");
...
//        a.setUserAdSn(pp);
        EasyHttp.post(this).api(a).request(new OnHttpListener<HttpData>() {
            @Override
