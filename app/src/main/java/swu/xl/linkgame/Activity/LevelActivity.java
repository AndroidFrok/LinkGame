...
public class LevelActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initData() {
        token = MmkvUtil.getString(MmkvUtil.Token, "");
        ...
        level_layout.post(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < levels.size(); i++) {
                    ...
                    //边距
                    int level_padding = (screenWidth - Constant.LEVEL_ROW_COUNT * PxUtil.dpToPx(Constant.LEVEL_SIZE, getApplicationContext())) / (Constant.LEVEL_ROW_COUNT + 1);

                    //创建视图
                    XLImageView xlImageView = new XLImageView(getApplicationContext(), LevelState.getState(levels.get(i).getL_new()));

                    //设置id
                    xlImageView.setId(i);

                    //布局参数
                    RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(PxUtil.dpToPx(Constant.LEVEL_SIZE, getApplicationContext()), PxUtil.dpToPx(Constant.LEVEL_SIZE, getApplicationContext()));

                    //添加约束
                    layoutParams.leftMargin = screenWidth * pager + level_padding + (level_padding + PxUtil.dpToPx(Constant.LEVEL_SIZE, getApplicationContext())) * pager_col;
                    layoutParams.topMargin = ScreenUtil.getStateBarHeight(getApplicationContext()) + PxUtil.dpToPx(Constant.LEVEL_TOP, getApplicationContext()) + level_padding + (level_padding + PxUtil.dpToPx(Constant.LEVEL_SIZE, getApplicationContext())) * pager_row;
